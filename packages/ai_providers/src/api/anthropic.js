...
async function fetchStream(stream, model, context, options) {
    const output = {
        role: "assistant",
        content: [],
        provider: "anthropic",
        ...
        for await (const event of iterateAnthropicEvents(response, options?.signal)) {
            if (event.type === "message_start") {
                ...
                if (event.delta.stop_reason) {
                    const stopReasonResult = stopReasonMapper(event.delta.stop_reason, event.delta.stop_details);
...
export const buildParams = (model, context, options) => {
...
function convertMessages(transformedMessages, allowEmptySignature = false) {
    const params = [];
    const loadedToolsNames = new Set();
    for (let i = 0; i < transformedMessages.length; i++) {
        ...
        if (msg.role === "user") {
            ...
            while (j < transformedMessages.length &&
                ...
                const converted = convertToolResult(transformedMessages[j]);
...
function stopReasonMapper(reason, stopDetails) {
    switch (reason) {
        case "end_turn":
        ...
        case "refusal":
            return {
                stopReason: "error",
                errorMessage: stopDetails?.explanation ||
